...
ВАЖНО: текст должен содержать ТОЛЬКО существо обращения. НЕ создавай и НЕ сохраняй в тексте «шапку»
адресата и отправителя (строки вида «Кому», «Прокурору города…», «старшему советнику юстиции», «от ФИО,
адрес, телефон, e-mail» и т.п.) — реквизиты адресата и заявителя оформляются ОТДЕЛЬНО в верхней части бланка.
Если такая шапка есть в исходном тексте — УДАЛИ её, начни сразу с существа (например, «Мы, нижеподписавшиеся…»).
...
ВАЖНО: текст должен содержать ТОЛЬКО существо обращения. НЕ создавай и НЕ сохраняй «шапку» адресата/отправителя
(«Кому», «Прокурору…», «от ФИО, адрес, телефон, e-mail» и т.п.) — реквизиты оформляются отдельно сверху бланка.
Если шапка есть в тексте — удали её, начни сразу с существа.
